...
fn get_grid() -> Vec<i32> {
    let mut ints = vec![];
    let items: Vec<&str> = GRID_STR.split_whitespace().collect();
    for i in items {
...
static GRID_SIZE: usize = ROW_LENGTH * ROW_LENGTH;
...
fn has_room_on_right(i: usize) -> bool {
    ROW_LENGTH - (i % ROW_LENGTH) >= WINDOW_SIZE
}

fn has_room_on_bottom(i: usize) -> bool {
    i + (ROW_LENGTH * (WINDOW_SIZE - 1)) < GRID_SIZE
}

fn has_room_on_top(i: usize) -> bool {
